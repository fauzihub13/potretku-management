...
export default function EditBookingPage() {
  ...
  const [form, setForm] = useState({
    clientName: '', clientEmail: '', clientPhone: '', eventType: '',
  ...
  useEffect(() => {
    Promise.all([api.get(`/bookings/${params.id}`), api.get('/services'), api.get('/team')])
      .then(([b, s, t]) => {
        const d = b.data;
        ...
        setForm({
          ...
          eventType: d.eventType, sessionDate: d.sessionDate?.split('T')[0] || '',
          sessionTime: d.sessionTime || '', location: d.location || '', packageName: d.packageName,
          packagePrice: d.packagePrice, totalAmount: d.totalAmount, dpAmount: d.dpAmount,
          notes: d.notes || '', freelancerId: d.freelancerId || '',
          driveAllPhotos: d.driveAllPhotos || '', driveRawPhotos: d.driveRawPhotos || '', driveEditedPhotos: d.driveEditedPhotos || ''
        });
        setServices(s.data);
        setTeam(t.data);
  ...
  const handleServiceSelect = (serviceId: string | null) => {
    ...
    if (s) {
      ...
      setForm(f => ({ ...f, packageName: s.name, packagePrice: s.price, totalAmount: s.price, dpAmount: Math.round(s.price * 0.3) }));
    }
  };
  ...
  if (fetching) return <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600" /></div>;

  return (
    <div className="space-y-4">
      <Button variant="ghost" onClick={() => router.back()}><ArrowLeft className="h-4 w-4 mr-2" /> Kembali</Button>
      <Card>
        <CardHeader><CardTitle>Edit Pemesanan</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="space-y-2"><Label>Nama Klien *</Label><Input value={form.clientName} onChange={e => update('clientName', e.target.value)} required /></div>
              <div className="space-y-2"><Label>Telepon</Label><Input value={form.clientPhone} onChange={e => update('clientPhone', e.target.value)} /></div>
              <div className="space-y-2"><Label>Email</Label><Input type="email" value={form.clientEmail} onChange={e => update('clientEmail', e.target.value)} /></div>
              ...
              <div className="space-y-2"><Label>Freelancer</Label><Select value={form.freelancerId} onValueChange={(v) => update('freelancerId', v || '')}><SelectTrigger><SelectValue placeholder="Pilih" /></SelectTrigger><SelectContent>{team.map(t=><SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-2"><Label>Tanggal *</Label><Input type="date" value={form.sessionDate} onChange={e => update('sessionDate', e.target.value)} required /></div>
              <div className="space-y-2"><Label>Jam Mulai *</Label><Select value={form.sessionTime} onValueChange={(v) => update('sessionTime', v || '')}><SelectTrigger><SelectValue placeholder="Pilih jam mulai" /></SelectTrigger><SelectContent className="max-h-64">{timeSlots.map(t=><SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
                ...
                <div className="sm:col-span-2">
                  <div className="flex items-center gap-2 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                    ...
                    <span className="text-sm font-medium text-purple-700 dark:text-purple-300">
                      Sesi: <span className="font-bold">{form.sessionTime}</span>
                      <span className="mx-1">→</span>
                      <span className="font-bold">{endTime}</span>
                      <span className="ml-2 text-purple-500">({durationText})</span>
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2"><Label>Total (Rp)</Label><Input type="number" value={form.totalAmount} onChange={e => update('totalAmount', Number(e.target.value))} /></div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button type="submit" className="bg-purple-600 hover:bg-purple-700" disabled={loading}>{loading ? 'Menyimpan...' : 'Simpan Perubahan'}</Button>
            </div>
          </form>
        </CardContent>
      </Card>
