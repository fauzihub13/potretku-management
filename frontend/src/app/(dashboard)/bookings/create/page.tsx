...
export default function CreateBookingPage() {
  ...
  useEffect(() => {
    if (selectedService) {
      setForm((f) => ({
        ...f,
        totalAmount: selectedService.price + addonsTotal,
        dpAmount: Math.round((selectedService.price + addonsTotal) * 0.3),
      }));
    }
  }, [addonsTotal]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.clientPhone) {
      const err = validatePhone(form.clientPhone);
      if (err) newErrors.clientPhone = err;
    }
    if (form.clientEmail) {
      const err = validateEmail(form.clientEmail);
      if (err) newErrors.clientEmail = err;
    }
    const dateErr = validateDate(form.sessionDate, "Tanggal sesi");
    if (dateErr) newErrors.sessionDate = dateErr;
    if (form.sessionTime) {
      const err = validateTime(form.sessionTime);
      if (err) newErrors.sessionTime = err;
    }
    const amountErr = validateNumber(form.totalAmount, "Total", 0);
    if (amountErr) newErrors.totalAmount = amountErr;
    if (!selectedServiceId) newErrors.package = "Pilih paket terlebih dahulu";
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.clientEmail)
                      setErrors((p) => {
                        const { clientEmail: _, ...r } = p;
                        return r;
                      });
                  }}
                />
                {errors.clientEmail && (
                  <p className="text-xs text-red-500">{errors.clientEmail}</p>
                )}
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Pilih freelancer">
                      {form.freelancerId &&
                        team.find((t) => t.id === form.freelancerId)?.name}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {team
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    update("sessionDate", e.target.value);
                    if (errors.sessionDate)
                      setErrors((p) => {
                        const { sessionDate: _, ...r } = p;
                        return r;
                      });
                  }}
                  required
                />
                {errors.sessionDate && (
                  <p className="text-xs text-red-500">{errors.sessionDate}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    update("sessionTime", v || "");
                    if (errors.sessionTime)
                      setErrors((p) => {
                        const { sessionTime: _, ...r } = p;
                        return r;
                      });
                  }}
                >
                ...
                {errors.sessionTime && (
                  <p className="text-xs text-red-500">{errors.sessionTime}</p>
                )}
              </div>
              {form.sessionTime &&
                selectedService &&
                  ...
                  <div className="sm:col-span-2 lg:col-span-3">
                    <div className="flex items-center gap-2 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                      ...
                      <span className="text-sm font-medium text-purple-700 dark:text-purple-300">
                        Sesi:{" "}
                        <span className="font-bold">{form.sessionTime}</span> →{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {addons.map((a) => {
                    ...
                    return (
                      <div
                        key={a.id}
                        className={`flex items-center gap-2 p-3 rounded-lg border transition-colors ${selected ? "border-purple-300 bg-purple-50 dark:bg-purple-900/20" : "border-zinc-200 hover:border-zinc-300"}`}
                      >
